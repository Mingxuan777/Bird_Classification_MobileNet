...
random.seed(123) # make the code result reproducible
...
def test(model, args):
    ...
    test_data = load_data.LoadDataset(train_data_path, val_data_path, test_data_path, args, train=False)
    test_dataloader = DataLoader(dataset=test_data, shuffle=False, batch_size=args.train_batch_size, num_workers=0)
    ...
    for ii, (input, label) in enumerate(test_dataloader): # ii is the batch number
        test_input = input
        test_label = label
        if torch.cuda.is_available(): # detect GPU and send data to GPU
        ...
        output = model(test_input) # calculate output result using validation data

        acc_batch = torch.mean(torch.eq(torch.max(output, 1)[1], test_label).float()) # calculate accuracy in a batch
        recall_batch = batch_recall(output, test_label)
        precision_batch = batch_Precision(output, test_label)

        acc_sum += acc_batch # sum up  batch accuracy
        recall_sum += recall_batch
        precision_sum += precision_batch

        print("------------- batch test accuracy: batch # %d -----------" %ii)
        print(acc_batch.detach().numpy())
        print("------------- batch recall rate: batch # %d -----------" % ii)
        print(recall_batch.detach().numpy())
        print("------------- batch precision rate: batch # %d -----------" % ii)
        print(precision_batch.detach().numpy())

        # confusionM.plot_confusion_matrix(np.argmax(test_label, axis=1), np.argmax(output, axis=1))


    acc_test = acc_sum / (ii + 1) # calculate average accuracy per batch
    recall_test = recall_sum / (ii+1)
    precision_test = precision_sum / (ii+1)
    print("-----------------------------------")
    print("-----------------------------------")
    print("------------- final test accuracy -----------")
    print(acc_test.detach().numpy())
    print("------------- final test recall rate -----------")
    print(recall_test.detach().numpy())
    print("------------- final test precision rate -----------")
    print(precision_test.detach().numpy())
    print("------------- end test -----------")
    print("-----------------------------------")
    print("-----------------------------------")
...
def setups():
    ...
    parser.add_argument('--train_data_path', type=str, default='./train', help="training data path")
    parser.add_argument('--val_data_path', type=str, default='./valid', help="validation data path")
    parser.add_argument('--model_path', type=str, default='./output/model.pt', help="output data path")
    parser.add_argument('--test_data_path', type=str, default='./test', help="test data path")
    ...
    parser.add_argument('--n_gpu', type=int, default=0,
                        help="training data path")  # larger the value, smaller the data
    parser.add_argument('--mode', type=str, default='test', help="train for training and test for testing")
...
def setup(args):
    ...
    if args.model == "MobileNet":
        model = models.mobilenet_v2(pretrained=True) # model to use
        model.classifier = nn.Sequential(nn.Dropout(p=0.2, inplace=False),
                                        nn.Linear(in_features=model.classifier[1].in_features, out_features=250, bias=True))
...
def train(args, model):
    ...
    train_data = load_data.LoadDataset(train_data_path, val_data_path, test_data_path, args, train=True)
    train_dataloader = DataLoader(dataset=train_data, shuffle=True, batch_size=args.train_batch_size, num_workers=0)

    # read validation data
    vali_data = load_data.LoadDataset(train_data_path, val_data_path, test_data_path, args, train=False)
    ...
    for epoch in range(epochs):
        for (input, target) in tqdm(train_dataloader): # use tqdm to visualize  training progress
            ...
            y = F.one_hot(target, num_classes=250)  # covert labels to one-hot encoding
