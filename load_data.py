...
def get_filepath(dir_root):
    ...
    for root, dirs, files in os.walk(dir_root): # walk through all files in the root directory and get their file directory
        for file in files:
            if '.DS_Store' in file: # skip DS_Store file in macos
                pass
            else:
...
class LoadDataset(data.Dataset):
    def __init__(self, train_root, val_root, test_root, args, transforms=None, train=True):
        self.train = train # accept training status
        imgs_in_train = get_filepath(train_root) # get image file
        imgs_in_val = get_filepath(val_root)
        imgs_in_test = test_root
        random.shuffle(imgs_in_train) # shuffle images
        imgs_num_train = len(imgs_in_train) # use special method __len__ to return the length of images.

        random.shuffle(imgs_in_val)
        imgs_num_val = len(imgs_in_val)
        ...
        if self.train is True: # train set
            self.imgs = imgs_in_train[:int(0.7 * imgs_num_train)] # list operation to cut training data
            # print('test') # for debug
        elif self.train is False:   # validation set
            self.imgs = imgs_in_val[:int(0.7 * imgs_num_val)] # list operation to cut validation data
            # print('test') # for debug
        elif self.test is True:
            self.imgs = imgs_in_test
    ...
    def __getitem__(self, index):
        ...
        label = int(img_path.split('/')[-2])  # use int(img_path.split('\\')[-2][1]) if you are using windows # get label, 0, 1, 2, ... 9
